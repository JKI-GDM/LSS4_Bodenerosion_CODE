...
saga$ta_hydrology$ls_factor_field_based(dem=paste(DATA.DIR,DEM.FILE,"_FILL.sgrd",sep=""),
                                          ls_factor=paste(DATA.DIR,DEM.FILE,"_nB-LS.sgrd",sep=""),
                                          method=0)
...
saga$ta_hydrology$ls_factor_field_based(dem=paste(DATA.DIR,DEM.FILE,"_FILL.sgrd",sep=""),
                                          ...
                                          ls_factor=paste(DATA.DIR,DEM.FILE,"_wB-LS.sgrd",sep=""),
                                          method=0)
...
for(i in 1:length(l.g)){
  r <- raster(paste(DATA.DIR,l.g[i],sep=""))
  writeRaster(r,paste(DATA.DIR,substr(l.g[i],1,nchar(l.g[i])-5),".asc",sep=""),overwrite=TRUE)
